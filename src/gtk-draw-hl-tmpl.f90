...
contains
  ...
  function hl_gtk_drawing_area_new(scroll, size, ssize, expose_event, &
       ...
       & cairo_status, hscroll_policy, vscroll_policy) result(plota_tmp)
    ...
    ! the function result and delete all references to plota_tmp
    type(c_ptr) :: plota_tmp
...
!!$GTK>=3.0!            & c_funloc(hl_gtk_drawing_area_expose_cb))
  ...
  function hl_gtk_drawing_area_get_gdk_pixbuf(area, x0, y0, xsize, ysize) &
    ...
    ! Workaround for ifort 15.0 bug to do with passing function results to 
    ! a value argument of a bind(c) procedure.  When the bug is fixed rename 
    ! the function result and delete all references to pixb_tmp
    type(c_ptr) :: pixb_tmp
